...
- `app/services/admin_config/` — DB-backed runtime config: `definitions.py` (the `ConfigDefinition` table: default, type, description, whether a change requires `CONFIRM`), `store.py` (get/set + audit log), `validation.py` (which changes require `CONFIRM`).
