...
class MarketSnapshot(Base):
    ...
    largest_bid_wall_distance_pct: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )
    largest_ask_wall_distance_pct: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )
    depth_buy_pressure_score: Mapped[float | None] = mapped_column(Float, nullable=True)
    depth_sell_pressure_score: Mapped[float | None] = mapped_column(
        Float, nullable=True
    )
    depth_order_book_signal: Mapped[str | None] = mapped_column(
