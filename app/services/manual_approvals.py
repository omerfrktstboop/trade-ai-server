...
async def queue_response(
    response, mode: SignalMode, source: str = "SCANNER"
) -> ManualApprovalRequest | None:
    if (
        response.action not in (SignalAction.BUY, SignalAction.SELL)
        or response.order_type != OrderType.LIMIT
        or response.qty <= 0
        or not response.price
        or response.price <= 0
    ):
        return None
    async with async_session_factory() as session:
        existing = (
            await session.execute(
                select(ManualApprovalRequest).where(
                    ManualApprovalRequest.request_id == response.request_id
                )
            )
        ).scalar_one_or_none()
        if existing:
        ...
        pending = (
            await session.execute(
                select(ManualApprovalRequest).where(
                    ManualApprovalRequest.symbol == response.symbol,
                    ManualApprovalRequest.action == response.action,
...
async def approve_request(
    request_id: int, approved_by: str, note: str | None = None
    ...
    try:
        ...
        outcome = await gateway_client.send_order(
            request_id=row.request_id,
            symbol=row.symbol,
            side=row.action,
            qty=row.qty,
            limit_price=row.price,
            mode=SignalMode.DEMO_LIVE.value,
        )
    ...
    async with async_session_factory() as session:
        ...
        row.admin_note = message
        existing = (
            await session.execute(
                select(OrderLog).where(OrderLog.request_id == row.request_id)
            )
        ).scalar_one_or_none()
        if existing is None:
            session.add(
...
async def reject_request(
    request_id: int, rejected_by: str, note: str | None = None
) -> ManualApprovalRequest:
    async with async_session_factory() as session:
        row = await session.get(ManualApprovalRequest, request_id)
        if row is None:
