...
async def build_performance_report(range_value: str = "7d", symbol: str | None = None) -> dict[str, Any]:
    ...
    categories = Counter(classify_block_reason(row.reason) for row in risks if not row.allow_order)
    statuses = Counter(row.status.upper() for row in orders)
    by_symbol = Counter(row.symbol for row in risks)
    return {"range": range_value, "symbol": symbol, "totalDecisions": len(risks),
        "buyCount": actions["BUY"], "sellCount": actions["SELL"], "waitCount": actions["WAIT"],
        "allowedOrders": sum(row.allow_order for row in risks), "blockedDecisions": sum(not row.allow_order for row in risks),
        "topBlockReason": categories.most_common(1)[0][0] if categories else "-",
        "topSymbols": by_symbol.most_common(10),
        "averageConfidence": round(sum(row.confidence for row in risks) / len(risks), 2) if risks else 0,
        "averageRiskScore": round(sum(row.risk_score for row in risks) / len(risks), 2) if risks else 0,
        "orderStatuses": dict(statuses), "ordersSent": statuses["SENT_PENDING"], "filledOrders": statuses["FILLED"],
        "rejectedOrders": statuses["REJECTED"], "errorOrders": statuses["ERROR"],
        "estimatedRealizedPnl": _estimated_pnl(orders), "pnlExperimental": True,
...
def _estimated_pnl(orders: list[OrderLog]) -> float:
    ...
    pnl = 0.0
    for row in sorted((r for r in orders if r.status.upper() == "FILLED"), key=lambda r: r.created_at):
        qty, price = row.qty, row.price or 0.0
        held, cost = costs.get(row.symbol, (0.0, 0.0))
        if row.action.upper() == "BUY":
            costs[row.symbol] = (held + qty, ((held * cost) + qty * price) / (held + qty))
