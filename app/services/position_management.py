...
async def record_position_management(request, raw: dict, response) -> None:
    ...
    async with async_session_factory() as session:
        ...
        action = str(
            raw.get("positionAction")
            or ("EXIT_FULL" if response.action.value == "SELL" else "HOLD")
        ).upper()
        action = action if action in _ACTIONS else "HOLD"
        qty = min(
            request.bot_position_qty,
            max(0.0, float(raw.get("positionQty") or response.qty or 0)),
        )
        if action == "EXIT_FULL":
            qty = request.bot_position_qty
        session.add(
            PositionManagementDecision(
                ...
                suggested_limit_price=response.price,
                stop_loss=response.stop_loss,
                take_profit=response.target_price,
