...
async def list_replay_candidates(symbol: str | None = None, since: datetime | None = None,
                                 until: datetime | None = None, limit: int = 100) -> list[str]:
    async with async_session_factory() as session:
        stmt = select(MarketSnapshot.request_id).order_by(MarketSnapshot.created_at.desc()).limit(limit)
        if symbol:
...
async def replay_request(request_id: str, profile_code: str | None = None,
                         mode: str | None = None) -> dict[str, Any] | None:
    async with async_session_factory() as session:
        snapshot = (await session.execute(select(MarketSnapshot).where(MarketSnapshot.request_id == request_id))).scalars().first()
        ai = (await session.execute(select(AiDecision).where(AiDecision.request_id == request_id))).scalars().first()
        original = (await session.execute(select(RiskDecisionModel).where(RiskDecisionModel.request_id == request_id))).scalars().first()
        ...
        request_data.update({"requestId": request_id, "symbol": snapshot.symbol, "timeframe": snapshot.timeframe,
            "lastPrice": snapshot.close, "open": snapshot.open, "high": snapshot.high, "low": snapshot.low,
            "volume": snapshot.volume, "rsi": snapshot.rsi, "ema20": snapshot.ema20, "ema50": snapshot.ema50,
            "macd": snapshot.macd, "macdSignal": snapshot.macd_signal, "botPositionQty": snapshot.position_qty,
            "totalAccountQty": snapshot.total_account_qty, "lockedLongTermQty": snapshot.locked_long_term_qty,
        ...
        replay = RiskEngine(config).evaluate(request, dict_to_risk_decision(ai.raw_response, request), request.market_regime)
        return {"requestId": request_id, "symbol": snapshot.symbol,
            ...
            "replayAction": replay.action.value, "replayAllowOrder": replay.allow_order,
            "replayReason": replay.reason, "profileCode": profile_code, "mode": request.mode.value}


async def replay_batch(profile_code: str | None = None, since: datetime | None = None,
                       until: datetime | None = None, symbols: list[str] | None = None,
                       limit: int = 100, mode: str | None = None) -> dict[str, Any]:
    ...
    return {"totalEvaluated": len(results), "replayAllowedOrderCount": sum(r["replayAllowOrder"] for r in results),
            "originalAllowedOrderCount": sum(r["originalAllowOrder"] for r in results),
            "changedCount": sum((r["originalAction"], r["originalAllowOrder"]) != (r["replayAction"], r["replayAllowOrder"]) for r in results),
            "results": results}
