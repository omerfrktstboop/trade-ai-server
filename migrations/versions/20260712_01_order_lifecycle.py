...
_NEW_COLUMNS = {
    ...
    "state": sa.Column(
        "state", sa.String(32), server_default="RESERVED", nullable=False
    ),
    "order_type": sa.Column(
        "order_type", sa.String(16), server_default="LIMIT", nullable=False
    ),
    "rounded_limit_price": sa.Column("rounded_limit_price", sa.Float()),
    "filled_qty": sa.Column(
        "filled_qty", sa.Float(), server_default="0", nullable=False
    ),
    "last_fill_qty": sa.Column(
        "last_fill_qty", sa.Float(), server_default="0", nullable=False
    ),
    "avg_price": sa.Column("avg_price", sa.Float()),
    "reservation_created_at": sa.Column(
        "reservation_created_at", sa.DateTime(timezone=True)
    ),
...
def _create_order_logs() -> None:
    op.create_table(
        ...
        sa.Column("status", sa.String(32), server_default="RESERVED"),
        sa.Column("mode", sa.String(16), server_default="PAPER"),
        sa.Column("matrix_message", sa.Text()),
        sa.Column(
            "created_at", sa.DateTime(timezone=True), server_default=sa.func.now()
        ),
        *[column.copy() for column in _NEW_COLUMNS.values()],
        sa.UniqueConstraint("request_id", name="uq_order_logs_request_id"),
    )
...
def _merge_duplicates(bind, columns: set[str]) -> None:
    duplicates = (
        bind.execute(
            sa.text(
                "SELECT request_id FROM order_logs GROUP BY request_id HAVING COUNT(*) > 1"
            )
        )
        .scalars()
        .all()
    ...
    for request_id in duplicates:
        rows = list(
            bind.execute(
                sa.text("SELECT * FROM order_logs WHERE request_id=:rid ORDER BY id"),
                {"rid": request_id},
            ).mappings()
        )
        winner = max(
            rows,
            key=lambda row: (
                _RANK.get(str(row.get("status") or "").upper(), 0),
        ...
        messages = " | ".join(
            dict.fromkeys(
                str(row.get("matrix_message"))
                for row in rows
                if row.get("matrix_message")
            )
        )
        updates = {
            "status": winner.get("status"),
            "qty": max(float(row.get("qty") or 0) for row in rows),
            "matrix_message": messages or winner.get("matrix_message"),
        }
        if "order_id" in columns:
            updates["order_id"] = next(
                (row.get("order_id") for row in reversed(rows) if row.get("order_id")),
                None,
            )
        assignments = ", ".join(f"{key}=:{key}" for key in updates)
        bind.execute(
            sa.text(f"UPDATE order_logs SET {assignments} WHERE id=:id"),
            {**updates, "id": winner["id"]},
        )
        bind.execute(
            sa.text("DELETE FROM order_logs WHERE request_id=:rid AND id<>:id"),
            {"rid": request_id, "id": winner["id"]},
        )
...
def upgrade() -> None:
    ...
    if "order_logs" not in tables:
        ...
        if "updated_at" not in columns:
            bind.execute(
                sa.text(
                    "UPDATE order_logs SET updated_at=COALESCE(created_at, CURRENT_TIMESTAMP)"
                )
            )
            with op.batch_alter_table("order_logs") as batch:
                batch.alter_column(
        ...
        uniques = {
            item.get("name") for item in inspector.get_unique_constraints("order_logs")
        }
        if "uq_order_logs_request_id" not in uniques:
            with op.batch_alter_table("order_logs") as batch:
                batch.create_unique_constraint(
                    "uq_order_logs_request_id", ["request_id"]
                )
    inspector = sa.inspect(bind)
    indexes = {item["name"] for item in inspector.get_indexes("order_logs")}
    if "ix_order_logs_state_updated" not in indexes:
        op.create_index(
            "ix_order_logs_state_updated", "order_logs", ["status", "updated_at"]
        )
    if (
        "ix_order_logs_order_id_not_null" not in indexes
        and bind.dialect.name == "postgresql"
    ):
        op.create_index(
            "ix_order_logs_order_id_not_null",
            "order_logs",
            ["order_id"],
            unique=False,
    ...
    if "manual_approval_requests" in set(inspector.get_table_names()):
        bind.execute(
            sa.text(
                "UPDATE manual_approval_requests SET status='PENDING_APPROVAL' WHERE status='PENDING'"
            )
        )
...
def downgrade() -> None:
    ...
    if "manual_approval_requests" in set(inspector.get_table_names()):
        bind.execute(
            sa.text(
                "UPDATE manual_approval_requests SET status='PENDING' WHERE status='PENDING_APPROVAL'"
            )
        )
    indexes = {item["name"] for item in inspector.get_indexes("order_logs")}
    for name in ("ix_order_logs_order_id_not_null", "ix_order_logs_state_updated"):
        if name in indexes:
            op.drop_index(name, table_name="order_logs")
    with op.batch_alter_table("order_logs") as batch:
        uniques = {
            item.get("name") for item in inspector.get_unique_constraints("order_logs")
        }
        if "uq_order_logs_request_id" in uniques:
            batch.drop_constraint("uq_order_logs_request_id", type_="unique")
        columns = {column["name"] for column in inspector.get_columns("order_logs")}
