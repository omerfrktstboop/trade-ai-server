...
def test_decision_cache_misses_when_any_context_fingerprint_changes():
    cache = DecisionCache()
    first = decision_context_fingerprint(
        {"symbol": "THYAO", "position": 0, "configHash": "a"}
    )
    second = decision_context_fingerprint(
        {"symbol": "THYAO", "position": 1, "configHash": "a"}
    )
...
def test_context_fingerprint_covers_akd_kap_and_timestamps():
    base = {
        "quoteEventUtc": datetime.now(UTC),
        "kapContext": {"risk": []},
        "brokerFlowContext": {"smartMoneyFlow": "UNKNOWN"},
        "profileCode": "NORMAL",
        "configHash": "1",
    }
...
async def test_active_risky_kap_is_found_outside_latest_ten(monkeypatch):
    ...
    assert any(
        row["title"] == "Active regulatory risk"
        for row in context["THYAO"]["riskEvents24h"]
    )
...
async def test_kap_and_akd_requests_are_single_flight(monkeypatch):
    class Gateway:
        ...
        async def get_institutions(self, symbol, **kwargs):
            self.akd_calls += 1
            await asyncio.sleep(0.01)
            return {"available": False, "period": "DAILY"}

    gateway = Gateway()
    monkeypatch.setattr(kap_service, "gateway_client", gateway)
    kap_service.invalidate_kap_cache()
    await asyncio.gather(
        kap_service.sync_kap_events("THYAO"), kap_service.sync_kap_events("THYAO")
    )
    await asyncio.gather(
        get_broker_flow_context(["THYAO"], gateway=gateway, config_version="v1"),
        get_broker_flow_context(["THYAO"], gateway=gateway, config_version="v1"),
    )
...
async def test_fill_event_invalidates_cached_buy(monkeypatch):
    ...
    await record_order_result(
        OrderResultRequest.model_validate(
            {
                "requestId": "fill-cache",
                "symbol": "THYAO",
                "action": "BUY",
                "orderQty": 1,
                "filledQty": 1,
