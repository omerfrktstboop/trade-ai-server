...
def test_live_alias_is_never_a_real_live_gateway_mode():
    ...
    assert (
        _effective_mode(
            "LIVE", profile, real_live_mode_allowed=True, real_live_armed=True
        )
        == "PAPER"
    )
...
def test_real_live_needs_both_explicit_phase_gates():
    ...
    assert (
        _effective_mode(
            "REAL_LIVE", profile, real_live_mode_allowed=False, real_live_armed=True
        )
        == "PAPER"
    )
    assert (
        _effective_mode(
            "REAL_LIVE", profile, real_live_mode_allowed=True, real_live_armed=False
        )
        == "PAPER"
    )
...
def test_gateway_refreshes_demo_account_before_orders_and_fails_closed():
    check = SOURCE.split("private string CheckModeGates", 1)[1].split(
        "private decimal GetSellableQty", 1
    )[0]
    assert "VerifyDemoAccountFresh()" in check
    verification = SOURCE.split("private bool VerifyDemoAccountFresh", 1)[1].split(
        "private void IncrementDailyTradeCount", 1
    )[0]
