...
def _payload():
    return {
        ...
        "depthAnalysis": {
            ...
            "spreadPct": 0.08,
            "bidAskRatioTop5": 0.7,
            "bidAskRatioTop10": 0.45,
            "bidAskRatioTop25": 0.5,
            "imbalanceTop5": -0.2,
            "imbalanceTop10": -0.38,
            "imbalanceTop25": -0.33,
            "buyPressureScore": 25,
            "sellPressureScore": 85,
...
def test_reliable_strong_sell_depth_blocks_buy_but_not_sell():
    ...
    assert (
        "depth" in engine._technical_feature_block_reason(req, SignalAction.BUY).lower()
    )
...
def test_gateway_depth_contract_is_shared_and_capped():
    ...
    build = source.split("private MarketDataPayload BuildMarketData", 1)[1].split(
        "private void InitializeIndicators", 1
    )[0]
