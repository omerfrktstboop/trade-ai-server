...
def test_runtime_mode_is_authoritative_and_mismatch_rejected():
    handler = _method("private async Task HandleOrderAsync", "private string CheckModeGates")
...
def test_market_data_reads_do_not_mutate_close_history():
    build = _method("private MarketDataPayload BuildMarketData", "private void InitializeIndicators")
...
def test_pending_idempotency_reconciliation_and_kap_contracts():
    ...
    kap = _method("private async Task HandleKapAsync", "private static bool IsKapLikeNews")
    assert "AddHours(-lookbackHours)" in kap


def test_position_snapshot_is_published_by_reference_swap():
    load = _method("private void LoadRealPositionsSnapshot", "private void UpdatePositionCache")
...
def test_bar_history_requires_a_real_bar_timestamp():
    update = _method("private void UpdateOhlcvSnapshotFromBarData", "private string ResolveBarEventSymbol")
