...
async def _approval_with_config(**overrides):
    ...
    async with async_session_factory() as session:
        ...
        row = ManualApprovalRequest(
            request_id="gated", symbol="THYAO", action="BUY", qty=1,
...
def test_approval_gates_do_not_call_gateway(
    ...
    async def run():
        await drop_all(); await init_db()
        gateway = FakeGateway()
        monkeypatch.setattr(approvals_service.settings, "manual_approval_allow_orders", setting_enabled)
...
def test_approval_success_sends_only_demo_live_limit(monkeypatch):
    async def run():
        await drop_all(); await init_db()
        gateway = FakeGateway()
        monkeypatch.setattr(approvals_service.settings, "manual_approval_allow_orders", True)
        monkeypatch.setattr(approvals_service, "gateway_client", gateway)
