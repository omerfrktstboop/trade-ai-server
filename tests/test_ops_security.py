...
async def test_scoped_tokens_are_not_interchangeable(monkeypatch):
    ...
    assert await verify_evaluation_token(
        HTTPAuthorizationCredentials(scheme="Bearer", credentials="evaluation-secret")
    )
    with pytest.raises(Exception):
        await verify_gateway_token(
            HTTPAuthorizationCredentials(
                scheme="Bearer", credentials="evaluation-secret"
            )
        )
    assert await verify_admin_token(
...
def test_gateway_masks_account_ids_and_migration_merges_duplicates():
    ...
    migration = (
        root / "migrations" / "versions" / "20260712_01_order_lifecycle.py"
    ...
    assert all(
        name in migration
        for name in ("order_qty", "limit_price", "state", "error_message")
    )
