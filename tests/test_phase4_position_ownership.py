...
def test_gateway_sellable_is_clamped_to_bot_owned_and_account_free():
    method = SOURCE.split("private decimal GetSellableQty", 1)[1].split(
        "private OrderExecutionResult", 1
    )[0]
...
def test_incomplete_empty_snapshot_does_not_clear_cache():
    method = SOURCE.split("private void LoadRealPositionsSnapshot", 1)[1].split(
        "private void UpdatePositionCache", 1
    )[0]
...
def test_stale_fill_cannot_reduce_cumulative_ownership():
    assert transition(
        "PARTIALLY_FILLED", "PARTIALLY_FILLED", current_filled=40, incoming_filled=40
    )[0]
    assert not transition(
        "PARTIALLY_FILLED", "PARTIALLY_FILLED", current_filled=40, incoming_filled=20
    )[0]
